...
class Eval_Images(object):
    ...
    def __init__(self, renderer, cfg, points_batch_size=100000, use_learnt_poses=True, use_learnt_focal=True,
                 device=None, render_type=None, c2ws=None, img_list=None):
    ...
    def eval_images(self, data, render_dir, fxfy, lpips_vgg_fn, logger, min_depth=0.1, max_depth=20, it=0):
        ...
        with torch.no_grad():
            rgb_pred = []
            depth_pred = []
            for i, (pixels_i, depth_i) in enumerate(zip(torch.split(pixels, self.points_batch_size, dim=1),
                                                        torch.split(depth_input, self.points_batch_size, dim=1))):
