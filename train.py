...
def train(cfg):
    ...
    if cfg['pose']['learn_pose']:
        ...
        if not auto_scheduler:
            scheduler_pose = torch.optim.lr_scheduler.MultiStepLR(optimizer_pose,
                                                                  milestones=list(range(scheduling_start,
                                                                                        scheduling_epoch + scheduling_start,
                                                                                        100)),
                                                                  gamma=cfg['training']['scheduler_gamma_pose'],
                                                                  last_epoch=epoch_it)
    else:
        optimizer_pose = None
        pose_param_net = None
    ...
    if cfg['distortion']['learn_distortion']:
        distortion_net = mdl.Learn_Distortion(n_views, cfg['distortion']['learn_scale'],
                                              cfg['distortion']['learn_shift'], cfg).to(device=device)
        ...
        try:
            ...
            load_dict = checkpoint_io_distortion.load(distortion_load_dir,
                                                      load_model_only=cfg['training']['load_ckpt_model_only'])
        except FileExistsError:
            load_dict = dict()
        epoch_it = load_dict.get('epoch_it', -1)
        if not auto_scheduler:
            scheduler_distortion = torch.optim.lr_scheduler.MultiStepLR(optimizer_distortion,
                                                                        milestones=list(range(scheduling_start,
                                                                                              10000 + scheduling_start,
                                                                                              100)),
                                                                        gamma=cfg['training'][
                                                                            'scheduler_gamma_distortion'],
                                                                        last_epoch=epoch_it)
    else:
    ...
    if cfg['pose']['learn_focal']:
        if cfg['pose']['init_focal_type'] == 'gt':
            ...
            init_focal = None
        focal_net = mdl.LearnFocal(cfg['pose']['update_focal'], cfg['pose']['fx_only'],
                                   order=cfg['pose']['focal_order'], init_focal=init_focal).to(device=device)
        ...
        epoch_it = load_dict.get('epoch_it', -1)
        if not auto_scheduler:
            scheduler_focal = torch.optim.lr_scheduler.MultiStepLR(optimizer_focal, milestones=list(
                range(scheduling_start, scheduling_epoch + scheduling_start, 100)),
                                                                   gamma=cfg['training']['scheduler_gamma_focal'],
                                                                   last_epoch=epoch_it)
    else:
    ...
    while epoch_it < (scheduling_start + scheduling_epoch):
        ...
        for batch in train_loader:
            ...
            if (checkpoint_every > 0 and (it % checkpoint_every) == 0):
                ...
                checkpoint_io.save('model.pt', epoch_it=epoch_it, it=it,
                                   loss_val_best=metric_val_best, scheduling_start=scheduling_start,
        ...
        if not auto_scheduler:
            ...
            if epoch_it < scheduling_start:
                ...
                new_lr = cfg['training']['learning_rate'] * (
                            (cfg['training']['scheduler_gamma']) ** int((epoch_it - scheduling_start) / 10))
                for param_group in optimizer.param_groups:
                    param_group['lr'] = new_lr
                if cfg['pose']['learn_pose']:
                    new_lr_pose = cfg['training']['pose_lr'] * (
                                (cfg['training']['scheduler_gamma_pose']) ** int((epoch_it - scheduling_start) / 100))
                    for param_group in optimizer_pose.param_groups:
                        param_group['lr'] = new_lr_pose
                if cfg['pose']['learn_focal']:
                    new_lr_focal = cfg['training']['focal_lr'] * (
                                (cfg['training']['scheduler_gamma_focal']) ** int((epoch_it - scheduling_start) / 100))
                    for param_group in optimizer_focal.param_groups:
                        param_group['lr'] = new_lr_focal
                if cfg['distortion']['learn_distortion']:
                    new_lr_distortion = cfg['training']['distortion_lr'] * (
                                (cfg['training']['scheduler_gamma_distortion']) ** int(
                            (epoch_it - scheduling_start) / 100))
